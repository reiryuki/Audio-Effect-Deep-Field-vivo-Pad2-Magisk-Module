...
if echo "$PROP" | grep -q m; then
  ...
  sed -i 's|ro.vivo.global.effect true|ro.vivo.global.effect false|g' $MODPATH/service.sh
  ui_print " "
else
  APPS=AudioFX
  hide_app
...
if [ "`grep_prop vafx.voice $OPTIONALS`" == 1 ]; then
  ui_print "- libvoicegain.so will be applied to voice_call, tts,"
  ui_print "  accessibility, assistant, & call_assistant stream"
  sed -i 's|#z||g' $FILE
  ui_print " "
fi
