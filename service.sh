...
PROP=ro.vivo.product.overseas
if [ ! "`getprop $PROP`" ]; then
  resetprop -n $PROP no
fi
PROP=ro.vivo.product.platform
if [ ! "`getprop $PROP`" ]; then
  resetprop -n $PROP MTK8798
fi
PROP=ro.vivo.product.solution
if [ ! "`getprop $PROP`" ]; then
  resetprop -n $PROP MTK
fi
resetprop -n ro.vivo.global.effect true
resetprop -n debug.music 1
